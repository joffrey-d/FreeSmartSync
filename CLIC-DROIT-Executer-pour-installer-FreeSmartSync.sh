...
"$APP_NAME — Réinstallation"
        ...
        uninstall)
            popup_question \
"Voulez-vous supprimer AUSSI votre configuration et vos profils ?
(dossiers synchronisés, répertoire de sauvegarde, profils de téléphones)

Si vous répondez NON, ils seront conservés et retrouvés
automatiquement lors d'une réinstallation." \
"$APP_NAME — Désinstallation"
            if [ $? -eq 0 ]; then
                ...
                # Conserver la config — mais retirer setup_done pour forcer le wizard
                # au prochain lancement si on réinstalle
                if [ -f "$HOME/.config/freesmartsync/config.json" ]; then
                    # Garder les profils intacts, juste noter la désinstallation
                    python3 -c "
import json, os
cfg_path = os.path.expanduser('~/.config/freesmartsync/config.json')
try:
    with open(cfg_path) as f: cfg = json.load(f)
    cfg['setup_done'] = False  # Force le wizard à la prochaine install
    with open(cfg_path, 'w') as f: json.dump(cfg, f, indent=2)
except: pass
" 2>/dev/null || true
                fi
                popup_info "✅ FreeSmartSync désinstallé.
Configuration et profils conservés.
Ils seront retrouvés automatiquement lors d'une réinstallation." "$APP_NAME"
            fi
            rm -rf "$INSTALL_DIR"
            rm -f "$DESKTOP_FILE"
...
if [ -n "$DESKTOP_DIR" ]; then
    ...
    # Fallback universel : chmod +x suffit sur KDE/XFCE
fi
