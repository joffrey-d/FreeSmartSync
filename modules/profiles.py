...
class ProfileEditDialog(QtWidgets.QDialog):
    ...
    def __init__(self, name, profile, lang="fr", parent=None):
        ...
        def add_via_adb():
            ...
            # Si pas de device_id, détecter automatiquement via adb devices
            if not device_id:
                try:
                    r = _sp.run(["adb", "devices"],
                                stdout=_sp.PIPE, stderr=_sp.PIPE, text=True, timeout=5)
                    devices = []
                    ...
                    if devices:
                        ...
                        QtWidgets.QMessageBox.warning(
                            self,
                            "Téléphone non connecté" if lang == "fr" else "Phone not connected",
                            "Branchez votre téléphone avec le débogage USB activé,"
                            " puis réessayez."
                            if lang == "fr" else
                            "Connect your phone with USB debugging enabled, then retry."
                        )
                        return
                except Exception:
                    QtWidgets.QMessageBox.warning(
                        self, "ADB",
                        "ADB non disponible. Vérifiez qu'il est installé."
                        if lang == "fr" else
                        "ADB not available. Check that it is installed."
                    )
                    return
