...
## v0.8.6.7 — Actuelle
**Correction icône dans les fenêtres Linux (Cinnamon / GNOME / KDE)**

Cause identifiée : le gestionnaire de fenêtres Linux identifie les fenêtres par
leur "classe" WM. Sans correspondance entre le .desktop et le code Python,
toutes les fenêtres héritent de l'icône générique Python (le fameux "W").

Corrections :
- `CLIC-DROIT-...sh` : ajout de `StartupWMClass=freesmartsync` dans le .desktop installé
- `CLIC-DROIT-...sh` : `Exec=python3 "$INSTALL_DIR/freesmartsync.py"` (simplifié)
- `freesmartsync.py` : `setApplicationName("freesmartsync")` avant `QApplication()`
- `freesmartsync.py` : `setApplicationDisplayName("FreeSmartSync")` pour l'affichage
  → Ces 3 valeurs correspondent exactement : classe WM = nom app = StartupWMClass

---

## v0.8.6.6
**Icône et fenêtres**
- Icône revue : flèches avec pointes intégrées au corps (pas de triangle séparé)
  Fond émeraude dégradé, flèche blanche (Tel→PC) + flèche semi-transparente (PC→Tel)
- Icône désormais visible dans **toutes** les fenêtres (barre de titre, alt-tab) :
  fenêtre principale, wizard, ReconnectDialog, MiniWizard, ProfileDialog,
  ProfileEditDialog, PreviewDialog, SummaryDialog, AboutDialog
  → Utilise `QApplication.setWindowIcon()` pour propagation automatique

---

## v0.8.6.5
**Nouvelle identité visuelle**
- Nouvelle icône : cercle vert, smartphone centré, flèches de synchronisation
- Icônes d'animation (4 frames) basées sur la nouvelle identité
- Icône visible correctement dans toutes les fenêtres (plus de "W" générique)
- Fichier .ico Windows inclus pour les raccourcis Bureau
...
## v0.8.6.5
**Nouvelle identité visuelle + MAJ manuelle + icône dans toutes les fenêtres**
- Nouvelle icône premium : fond vert émeraude dégradé, smartphone blanc centré,
  deux grandes flèches continues (sans pointe détachée), finition glassmorphism
- Frames d'animation 64px pour le tray (rotation des flèches pendant la sync)
- **Icône FSS visible dans toutes les fenêtres** (fenêtre principale, wizard, profils, dialogs)
- Bouton **🔔 Mises à jour** dans la barre principale : vérifie GitHub à la demande
  sans attendre le démarrage automatique
- Le bouton affiche "À jour ✅" si aucune nouvelle version n'est disponible (feedback visible)

---

## v0.8.6.2
**Restauration explorateur ADB fonctionnel**
- Code de détection des dossiers ADB restauré depuis v0.8.5.8-fix13 (version connue fonctionnelle)
- Détection automatique du téléphone branché au démarrage via `adb devices`
- `freesmartsync.py` nettoyé : commentaires clairs, structure lisible

---

## v0.8.6.1
**Tentative de correction explorateur ADB**
- Réécriture de `adb_explorer.py` avec `for f in path/* ; do [ -d $f ]` (shell POSIX)
- Non fonctionnel sur certains appareils — remplacé par v0.8.6.2

---

## v0.8.6
**Explorateur ADB — 3 méthodes en cascade**
- Méthode 1 : `ls -F` (universel Android/busybox — dossiers identifiés par le `/` final)
- Méthode 2 : `find -maxdepth 1 -type d` (fallback)
- Méthode 3 : `ls` + `test -d` fichier par fichier (dernier recours)
→ Fonctionne sur tous les appareils Android

**Profils**
- Profil actif sauvegardé avant de switcher → dossiers conservés
- Label profil mis à jour immédiatement après sélection
- Device ADB détecté automatiquement à l'ouverture de l'explorateur dans les profils

**Corrections**
- Icône Fedora Cinnamon : `Exec` avec guillemets pour les chemins avec espaces
- Boucle infinie `itemChanged` → verrou `_loading` + connexion après chargement

---

## v0.8.5.8-fix14 — Actuelle
**Correction explorateur ADB**
- Sous-dossiers maintenant visibles (utilisation de `find -maxdepth 1 -type d`
  au lieu de `ls -d /*/` qui échoue sur certains appareils Android/KDE)
- Suppression de la boucle infinie entre `setExpanded` → `_on_expand` → `_load_children`
  (chargement fait AVANT setExpanded)
- Signal `itemChanged` connecté APRÈS le chargement initial (évite les déclenchements parasites)
- Verrou `_loading` pour bloquer tous les signaux pendant les opérations d'arbre
