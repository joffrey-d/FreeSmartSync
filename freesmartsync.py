...
=======
# freesmartsync.py — Point d'entrée FreeSmartSync v0.8.5.6
>>>>>>> Stashed changes
...
class ReconnectDialog(QtWidgets.QDialog):
    """
    Assistant de reconnexion affiché au relancement.
    ...
    def __init__(self, lang="fr", parent=None):
        ...
        # Sélecteur de profil si profils existants
        ...
        if profiles:
            profile_lbl = QtWidgets.QLabel(
                "👤 Profil à utiliser :" if lang == "fr" else "👤 Profile to use:"
            )
            ...
            self.profile_combo.addItem(
                "— Config actuelle —" if lang == "fr" else "— Current config —",
                None
            )
...
class MiniWizard(QtWidgets.QDialog):
    """
    Wizard rapide de reconnexion :
    - Page 0 : procédure débogage USB par marque
    - Page 1 : connexion USB animée + détection
    """
    ...
    def __init__(self, config, lang="fr", parent=None):
        ...
        self.config  = config
        self.lang    = lang
        self._launched = False  # Garde contre double lancement
    ...
    def _show_page(self, idx):
        ...
        self.progress_lbl.setText(f"{idx + 1} / {total}")
        ...
        if idx == 0:
            ...
            # Bouton grisé jusqu'à détection du téléphone
        ...
        if self.current == 0:
            ...
            if not device_id:
                QtWidgets.QMessageBox.warning(
                    ...
                    "Branchez votre téléphone et attendez qu'il soit détecté."
    ...
    def _skip(self):
        """Passe directement sans attendre la détection."""
        self.page_usb.stop_timers()
    ...
    def _launch_with_device(self, device_id):
        """Lance FreeSmartSync avec le device_id détecté."""
        self.page_usb.stop_timers()
        if not self._launched:
            self._launched = True
...
def main():
    ...
    if is_first_run():
        ...
        if result == QtWidgets.QDialog.Accepted:
            ...
            # Sécurité : si fermé sans lancer (croix)
            if not launched[0]:
