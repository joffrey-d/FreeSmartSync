...
def main():
    # ── Identification Linux : lie les fenêtres à l'icône du .desktop ──
    if sys.platform.startswith("linux"):
        try:
            # Doit correspondre exactement à StartupWMClass dans le .desktop
            QtCore.QCoreApplication.setApplicationName("freesmartsync")
        except Exception:
            pass

    app = QtWidgets.QApplication(sys.argv)
    app.setApplicationName("freesmartsync")        # identique à StartupWMClass
    app.setApplicationDisplayName("FreeSmartSync") # nom affiché à l'utilisateur
    app.setOrganizationName("FreeSmartSync")
    app.setQuitOnLastWindowClosed(False)

    # Charger l'icône et l'appliquer à toute l'application
    if os.path.exists(_ICON_PATH):
        # Ajouter le dossier assets dans le search path des thèmes Qt
        assets_dir = os.path.dirname(_ICON_PATH)
        QtGui.QIcon.setThemeSearchPaths(
            QtGui.QIcon.themeSearchPaths() + [assets_dir]
        )
        # Charger depuis le fichier (plus fiable que fromTheme sur toutes les distros)
        _app_icon = QtGui.QIcon(_ICON_PATH)
        # Ajouter toutes les tailles disponibles pour KDE/Plasma
        for sz in ["icon_256.png", "icon_anim1.png"]:
            extra = os.path.join(assets_dir, sz)
