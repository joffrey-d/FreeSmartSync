...
class Worker(QtCore.QThread):
    ...
    def _find_adb(self):
        """Trouve adb — PATH ou dossier tools/ local (Windows)."""
        import shutil, sys
        if shutil.which("adb"):
            return "adb"
        # Windows : chercher tools/adb.exe à côté du .exe
        base = getattr(sys, "_MEIPASS", os.path.dirname(
            os.path.dirname(os.path.abspath(__file__))
        ))
        local = os.path.join(base, "tools", "adb.exe")
        if os.path.exists(local):
            return local
        return "adb"

    def run_cmd(self, cmd, timeout=60):
        # Remplacer "adb" par le bon chemin si nécessaire
        if cmd and cmd[0] == "adb":
            cmd = [self._find_adb()] + list(cmd[1:])
        try:
